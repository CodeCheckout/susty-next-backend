...
export const addProducts = async(req, res) => {
    ...
    await Seller.create(newSellerItem).then((item) => {
        return res.status(200).json({
            success: true,
...
export const addProductAsSeller = async(req, res) => {
    ...
    await Seller.findOneAndUpdate({userId: userId}, {$push: { products: productId }}).then((updatedItem) => {
        return res.status(200).json({
            success: true,
            message: "Product added successfully",
            updatedItem
...
export const addMySellings = async(req, res) => {
    ...
    await Seller.findOneAndUpdate({userId: userId}, {$push: { mySellings: productId }}).then((updatedItem) => {
        return res.status(200).json({
            success: true,
